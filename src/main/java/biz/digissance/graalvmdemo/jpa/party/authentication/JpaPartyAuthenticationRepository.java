...
public interface JpaPartyAuthenticationRepository extends JpaRepository<JpaPartyAuthentication, Long> {
    ...
    @Query("from JpaEmailPasswordPartyAuthentication auth inner join fetch auth.party party inner join fetch party.roles role inner join fetch role.type where auth.emailAddress = ?1")
